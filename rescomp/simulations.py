...
def _kuramoto_sivashinsky_Bhatt(dimensions, system_size, dt, time_steps, starting_point, alpha=1, beta=1):
    ...
    Args:
        ...
        starting_point (np.ndarray): starting point for the simulation of shape
            (dimensions, )
        alpha (float): KS-eq parameter
        beta (float): KS-eq parameter
...
def _kuramoto_sivashinsky_alt(dimensions, system_size, dt, time_steps, starting_point, M=16, **kwargs):
    '''
    Github: Python implementation at: https://github.com/E-Renshaw/kuramoto-sivashinsky
    Args:
        dimensions:
        system_size:
        dt:
    ...
    if starting_point is None:
        x = np.transpose(np.conj(np.arange(1, N + 1))) / N
        u = np.cos(x / system_size) * (1 + np.sin(x / system_size))
    else:
        u = starting_point
    ...
    k = np.transpose(np.conj(np.concatenate((np.arange(0, N / 2), np.array([0]), np.arange(-N / 2 + 1, 0))))) / system_size
    L = k ** 2 - k ** 4
    E = np.exp(h * L)
    E_2 = np.exp(h * L / 2)
    # M = 16
